...
def loss_f(mesh_verts, mesh_faces, points, it):
    ...
    chamfer = kal.metrics.pointcloud.chamfer_distance(
        pred_points.unsqueeze(0), points.unsqueeze(0)
    ).mean()
...
def batch_subdivide_volume(tet_pos_bxnx3, tet_bxfx4):
    ...
    mid_points_pos = all_values[:, unique_edges.reshape(-1)].reshape(
        all_values.shape[0], -1, 2, all_values.shape[-1]
    ).mean(2)
...
class DMTetMesh(nn.Module):
    ...
    def get_mesh(self, return_loss=False, num_subdiv=None):
        ...
        verts_deformed = self.tet_v + torch.tanh(
            deform
        ) * self.grid_scale / 2    # constraint deformation to avoid flipping tets
        tet = self.tet_ind
        for i in range(num_subdiv):
            verts_deformed, _, tet = compact_tets(verts_deformed, sdf, tet)
            verts_deformed, tet = batch_subdivide_volume(
                verts_deformed.unsqueeze(0), tet.unsqueeze(0)
            )
    ...
    def init_mesh(self, mesh_v, mesh_f, init_padding=0.):
        ...
        ).to(self.device) - init_padding
        sdf_mesh_v, sdf_mesh_f = kal.ops.conversions.marching_tetrahedra(
            self.tet_v.unsqueeze(0), self.tet_ind, sdf_tet.unsqueeze(0)
        )
